    ...
    <div class="container mx-auto max-w-5xl px-4">
      ...
      <div class="flex items-start justify-between mb-8">
        <div class="flex items-center gap-6">
          <div style="background-image: linear-gradient(135deg, var(--color-primary), var(--color-secondary))" class="w-24 h-24 rounded-2xl flex items-center justify-center text-4xl font-bold text-white shadow-lg shadow-primary/20">
            {{ initials }}
          </div>
          <div>
      ...
      <div v-else-if="statistics" class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-slate-900/40 rounded-2xl p-6 border border-primary/10">
          <div class="text-slate-400 text-sm mb-2 uppercase tracking-wide font-medium">Best WPM</div>
          <div class="text-4xl font-bold text-primary">{{ statistics.best_wpm || 0 }}</div>
        </div>
...
onMounted(() => {
  setMeta({
    title: 'Profile - KeyFlow',
    description: 'View your typing statistics, track your progress, and see your recent activity on KeyFlow.',
    keywords: 'typing profile, typing statistics, typing progress, WPM stats'
  })
})
