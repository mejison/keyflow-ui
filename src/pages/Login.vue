    ...
    <div class="container mx-auto max-w-md px-4">
      <div class="bg-slate-800/40 backdrop-blur-md rounded-3xl shadow-2xl border border-slate-700/30 p-10">
        <div class="text-center mb-8">
          ...
          <p class="text-slate-400">Sign in to track your progress</p>
        </div>
        ...
        <div class="relative my-6">
          ...
          <div class="relative flex justify-center text-sm">
            <span class="px-2 bg-slate-800/40 text-slate-500">Or continue with email</span>
          </div>
        </div>
        ...
        <div class="mt-6 text-center">
          <p class="text-slate-400 text-sm">
            ...
            <RouterLink to="/signup" class="text-blue-400 hover:text-blue-300 font-medium">Sign up</RouterLink>
          </p>
        </div>
      </div>
    </div>
    ...
    if (err.response?.data) {
      ...
      if (responseData.data && typeof responseData.data === 'object') {
        const errorMessages = Object.entries(responseData.data)
          ...
          .join('; ')
        error.value = errorMessages || responseData.message || 'Login failed'
      }
      ...
      // Just message
        ...
        error.value = responseData.message || err.message || 'Login failed'
      }
    } else {
...
const handleOAuthLogin = async (provider: 'github' | 'google') => {
  ...
  try {
    ...
    if (err.response?.data) {
      ...
      error.value = err.message || 'OAuth login failed. Please try again.'
    }
  }
}
</script>
